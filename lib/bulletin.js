...
export default class Bulletin {
  ...
  updateStatus(taskName, status) {
    if (this.tasks[taskName].status === "running" && status !== "running") {
      this.tasks[taskName].ranLast = this.formatDate(new Date());
    }
  ...
  padEvenly(message, maxLength) {
    ...
    //   message = message.slice(message.length - 2);
    //   back = back + "\n";
    // }
  ...
  draw(isFirstTime) {
    ...
    const headers = [
      ...
      this.padEvenly("RAN LAST", maxDateLength),
    ];
    ...
    _.forEach(this.tasks, (task, name) => {
      ...
      const ranLast = this.padEvenly(task.ranLast, maxDateLength);

      // Body lines
      this.write(
        `\u2502 ${padName} \u2502 ${status} \u2502 ${ranLast} \u2502\n`
      );
    });
